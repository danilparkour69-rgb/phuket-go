...
Текущий фокус: довести публичный сайт `website` и основной путь заявки до локального 10/10. До этого не деплоим, не загружаем build output в облако, не создаем production ресурсы и не начинаем публикацию.
...
2026-07-02: начат финальный polish публичного сайта перед релизной подготовкой.

Фактическое состояние текущего website pass:

- `website` синхронизирует 569 медиафайлов из `docs/03-service-catalog/media/excursions/*/final/carousel` в `website/public/media`;
- generated media исключены из git через `.gitignore`;
- production build сайта больше не должен зависеть от `localhost`/`127.0.0.1` для изображений;
- главная получила реальный фото-hero;
- страница экскурсии получила более аккуратную мобильную галерею и header;
- локально проверялись `bun run --cwd website typecheck`, `bun run build:website`, `bun run typecheck`, `bun run build`;
- Playwright screenshots были сохранены в `.scratch/visual-audit/`.

Важно: текущий website polish находится в рабочем дереве и еще не был отдельно закоммичен/запушен. Не считать эти правки финально `[done]` в task map до commit/push.

Предыдущий закрытый блок:
...
- Telegram polish для партнера сделан: на новой партнерской заявке одна главная кнопка `✅ Принять заявку`, связь с клиентом отдельной строкой, после принятия показывается рабочая клавиатура `✅ В работе`, связь с клиентом, `💰 Оплата получена`, `🆘 Нужна помощь`, `❌ Отклонить`; callback-data `accept`, `paid`, `problem`, `decline` сохранены;
...
Следующий практичный блок: довести `website` до локального 10/10, затем повторить полный smoke заявки через website, backend, Telegram, Google Sheets и админку. Yandex/hosting/domain/docs возвращаются только после этого и отдельного решения владельца проекта.
...
Важно: старое решение "не добивать сейчас верстку" больше не актуально. Сейчас как раз добиваем публичный сайт до уверенного MVP-уровня: desktop/mobile, карточки, фильтры, CTA, фото, ошибки/успехи и отсутствие визуальных поломок.
...
3. Довести публичный сайт `website` до 10/10: главная, каталог, карточки, страницы экскурсий, статьи, FAQ, контакты, условия, mobile/desktop.
4. Проверить фото: сайт уже берет доступные фото из папки направления, но перед публикацией нужна ручная чистка и `approved` для финальных изображений.
5. Повторить полный локальный smoke заявки: website -> backend -> Telegram admin/partner -> Google Sheets -> admin queue.
6. Обновить task map/docs после финального состояния сайта.
7. Только после локального 10/10 и отдельного решения владельца проекта возвращаться к домену, хостингу и публикации.
8. Сжатый контекст для нового окна: `docs/00-project-control/context-compression.md`.
9. Инструкция Codex от владельца проекта сохранена: `docs/00-project-control/project-instructions-for-codex.md`.
10. Если пользователь просит разработку, сначала прочитать сохраненную инструкцию и применить ее с учетом приоритета `AGENTS.md`, шаблона `vibe` и актуальных документов `docs/`.
11. Код проекта вести с учетом сохраненной инструкции.
...
`Продолжаем Phuket Go. Локальная разработка уже начата. Сначала открой docs/00-project-control/context-compression.md, затем docs/00-project-control/project-instructions-for-codex.md, docs/00-project-control/NEXT_SESSION.md, docs/00-project-control/master-plan.md, docs/10-mvp-roadmap/development-handoff-map.md, docs/10-mvp-roadmap/immediate-pre-coding-actions.md и docs/10-mvp-roadmap/pre-coding-docs-consistency-review.md. Текущий фокус - довести website и путь заявки до локального 10/10. До этого не деплоим, не загружаем в облако и не начинаем публикацию. Фото не блокируют локальную разработку, но перед публикацией нужна ручная чистка. ASO зафиксировано как будущая задача мобильного приложения. При разработке используй сохраненную инструкцию Codex с учетом приоритета AGENTS.md, шаблона vibe и актуальных документов docs/.`
