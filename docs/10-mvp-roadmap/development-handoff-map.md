...
Локальная разработка уже начата; этот документ остается картой передачи решений в код.

## Цель

Этот документ нужен, чтобы при продолжении разработки Phuket Go не перечитывать всю папку `docs`.
...
Статус: локальная реализация идет; публичный сайт сейчас главный блок до 10/10.

Уже сделано:

- сайт собирает главную, каталог, страницы экскурсий, статьи, FAQ, контакты и условия;
- карточки и страницы используют подготовленную базу 54 экскурсий;
- фото берутся из `media-manifest.json` и `final/carousel`;
- добавлен локальный media sync: `website` копирует 569 файлов в `website/public/media`, а generated media не коммитятся;
- production build больше не зависит от `localhost`/`127.0.0.1` media URL;
- главная получила фото-hero;
- мобильная страница экскурсии получила более аккуратную галерею и header;
- `website` build/typecheck и общий build/typecheck проходили после текущего polish.

Что нужно закрыть сейчас:

- финальный visual pass главной, каталога, карточек, страниц экскурсий, статей, FAQ, контактов и условий на desktop/mobile;
- проверить, что нет наложений текста, пустых контейнеров, слишком слабых первых экранов, битых картинок и localhost-ссылок;
- довести фильтры, карточки, CTA, форму заявки и состояния ошибок/успеха до аккуратного MVP;
- вручную проверить фото перед публикацией: слабые/повторные/неподходящие изображения, лица людей, соответствие экскурсии, статус `approved` для финального релизного набора;
- повторить сквозной smoke сайта и заявки после финальной верстки.

Релизное правило: пока сайт и путь заявки не станут локально 10/10, не начинать деплой, загрузку в облако, настройку production ресурсов и публикацию.
...
1. Довести `website` до 10/10 по верстке, фото, CTA и мобильным состояниям.
2. После финальной верстки повторить полный путь заявки: public website -> backend -> Telegram admin/partner -> Google Sheets -> admin queue.
3. Решить по оставшимся MVP-поверхностям: пользовательский Telegram-бот, личный кабинет, баллы, расширенные разделы админки.
4. Обновить docs и task map под фактическое состояние.
5. Только после этого возвращаться к `docs/DEPLOYMENT.md` или `docs/YANDEX_CLOUD.md`.
...
- [ ] Довести публичный сайт и путь заявки до локального 10/10.
