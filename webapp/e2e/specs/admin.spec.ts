...
test('admin can open a lead and change its status from the UI', async ({ page }) => {
  ...
  const manualLeadName = `E2E Bike Client ${Date.now()}`
  ...
  try {
    ...
    const createLeadForm = page.getByRole('form', { name: 'Новая заявка' })
    await createLeadForm.getByRole('combobox', { name: 'Направление' }).click()
    await page.getByRole('option', { name: 'Аренда байков' }).click()
    await expect(createLeadForm.getByRole('combobox', { name: 'Экскурсия' })).toBeHidden()
    await createLeadForm.getByRole('combobox', { name: 'Партнер' }).click()
    await page.getByRole('option', { name: new RegExp(`E2E Partner ${publicNumber}`) }).click()
    await createLeadForm.getByLabel('Клиент').fill(manualLeadName)
    await createLeadForm.getByLabel('Телефон').fill('+66990007777')
    await createLeadForm.getByLabel('Количество').fill('1')
    await createLeadForm.getByLabel('Комментарий').fill('E2E bike rental lead')
    await createLeadForm.getByRole('button', { name: 'Создать заявку' }).click()
    await expect(page.getByText(manualLeadName).first()).toBeVisible()
    await expect(page.getByText('Аренда байков').first()).toBeVisible()
    await page.getByLabel('Поиск').fill(leadGroup)
    await page.getByRole('button', { name: 'Применить' }).click()
    await expect(page.getByText('1-14 из 14')).toBeVisible()
    await page.locator('#admin-lead-partner-filter').click()
    await page.getByRole('option', { name: new RegExp(`E2E Partner ${publicNumber}`) }).click()
    await page.getByRole('button', { name: 'Применить' }).click()
    await expect(page.getByText('1-14 из 14')).toBeVisible()
    ...
    expect(csvContent).toContain('lead_id,public_number,is_test,status')
